...
public class ElectrolysisPlantContainerMenu extends AbstractContainerMenu {
    public final ElectrolysisPlantBlockEntity blockEntity;
    private final ContainerLevelAccess access;
    private final ContainerData data;

    public ElectrolysisPlantContainerMenu(final int containerId, final Inventory playerInv, final FriendlyByteBuf data) {
        this(containerId, playerInv, getBlockEntity(playerInv, data), ContainerLevelAccess.NULL, new SimpleContainerData(3));
    }

    public ElectrolysisPlantContainerMenu(final int containerId,
                                          final Inventory playerInv,
                                          final ElectrolysisPlantBlockEntity blockEntity,
                                          final ContainerLevelAccess access,
                                          final ContainerData data) {
        super(ModMenuTypes.ELECTROLYSIS_PLANT.get(), containerId);
    ...
    public boolean stillValid(final Player player) {
        return AbstractContainerMenu.stillValid(this.access, player, ModBlocks.ELECTROLYSIS_PLANT.get());
    }

    private static ElectrolysisPlantBlockEntity getBlockEntity(final Inventory playerInventory, final FriendlyByteBuf data) {
        Objects.requireNonNull(playerInventory, "playerInventory cannot be null!");
        Objects.requireNonNull(data, "data cannot be null!");
        final BlockEntity blockEntity = playerInventory.player.level().getBlockEntity(data.readBlockPos());
        if (blockEntity instanceof ElectrolysisPlantBlockEntity block) {
            return block;
        }
