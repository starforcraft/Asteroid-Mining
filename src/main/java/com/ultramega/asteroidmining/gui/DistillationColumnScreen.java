...
public class DistillationColumnScreen extends AbstractContainerScreen<DistillationColumnContainerMenu> {
    ...
    protected void extractLabels(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY) {
        ...
        graphics.text(font, text, 35 - font.width(text) / 2, 62, -12566464, false);
    }
    ...
    public void extractBackground(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final float a) {
        ...
        graphics.blit(GUI_TEXTURED, BACKGROUND, this.leftPos + 34, this.topPos + 16, 101, 182, 0, 2, FLUID_BAR_HEIGHT, 256, 256);
    ...
    protected void extractTooltip(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY) {
        ...
        if (isHovering(34, 16, 6, FLUID_BAR_HEIGHT + 1, mouseX, mouseY)) {
            FluidContainerUtil.renderFluidTooltip(graphics, font, this.menu.blockEntity.fluidTank.getStackInTank(0), mouseX, mouseY);
        }
        if (isHovering(58, 16, 12, FLUID_BAR_HEIGHT + 1, mouseX, mouseY)) {
            FluidContainerUtil.renderFluidTooltip(graphics, font, this.menu.blockEntity.fluidTank.getStackInTank(1), mouseX, mouseY);
        }
        if (isHovering(121, 18, 6, ENERGY_BAR_HEIGHT + 1, mouseX, mouseY)) {
            FluidContainerUtil.renderFluidTooltip(graphics, font, this.menu.blockEntity.fluidTank.getStackInTank(2), mouseX, mouseY);
        }
    }
}
